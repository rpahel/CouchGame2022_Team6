...
public class ProjectileRaph : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collider)
    {
        if (collider.gameObject != owner.gameObject)
        {
            ...
            else if (collider.gameObject.layer == LayerMask.NameToLayer("CubeEdible") || collider.gameObject.layer == LayerMask.NameToLayer("Bedrock"))
            {
            ...
            if (hit && hit.transform.gameObject.layer != LayerMask.NameToLayer("Projectile"))
            {
                if(hit.transform.GetComponent<PlayerManager>() == owner)
                {
                    ...
                    if(ClosestAxis(impactedCubeToOwner, true).x == 0)
                    {
                        ...
                        Vector2 safe = position + (owner.GetComponent<Collider2D>().bounds.extents.y + .5f * GameManager.Instance.LevelGenerator.Echelle) * ClosestAxis(impactedCubeToOwner, true);
                        owner.PousseToiVers(safe + new Vector2(owner.transform.position.x - safe.x, 0));
                    }
                    else
                    {
                        Debug.DrawRay((Vector2)owner.transform.position - impactedCubeToOwner, impactedCubeToOwner, Color.blue, 5f);
                        Vector2 safe = position + (owner.GetComponent<Collider2D>().bounds.extents.x + .5f * GameManager.Instance.LevelGenerator.Echelle) * ClosestAxis(impactedCubeToOwner, true);
    ...
    Vector2 ApproxNormal(Vector2 normalToApproximate)
    {
        ...
        Debug.Log("Tu ne devrais pas voir �a.");
        return Vector2.up;
    }
